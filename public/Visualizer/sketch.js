...
let volume = 1; //range 0 to 1
let isDraggingVolume = false; //for volume slider sliding
...
function mousePressed() {
  ...
  // --- Volume slider click detection ---
  let centerX = width / 4;
  let centerY = -height / 2 + 60; // Must match drawUI
  let sliderWidth = playPauseButton.size * 2;
  let sliderHeight = 8;
  let sliderX = centerX - sliderWidth / 2;
  let sliderY = centerY - sliderHeight / 2;

  if (
    screenX > sliderX &&
    screenX < sliderX + sliderWidth &&
    screenY > sliderY &&
    screenY < sliderY + sliderHeight
  ) {
    volume = constrain((screenX - sliderX) / sliderWidth, 0, 1);
    song.setVolume(volume);
...
function drawUI() {
  ...
  // Skip forward icon (triangle pointing right with line)
  fill(180);
  noStroke();
  ...
  //  Volume slider
  push();
  // Position near top right quarter, but relative to canvas
  let centerX = width / 4;
  let centerY = -height / 2 + 60; // 60px down from top of canvas
  ...
  // Slider knob
  let knobX = sliderX + volume * sliderWidth;
  fill(50, 180);
  ellipse(knobX, sliderY, sliderHeight * 1.5);
